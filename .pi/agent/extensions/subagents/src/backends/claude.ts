...
const makeClaudeSession = (
  ...
  Effect.gen(function* () {
    ...
    // Pre-assign new Claude sessions so the recovery handle exists before the
    // subprocess starts. Restored sessions reuse their persisted native id.
    const nativeSessionId = recoveryMeta?.nativeSessionId ?? randomUUID();
    ...
    const state = {
      ...
      meta: {
        ...
        contextWindow:
          recoveryMeta?.contextWindow ?? CLAUDE_CONTEXT_WINDOW,
        nativeSessionId,
        sessionFilePath:
          recoveryMeta?.sessionFilePath ??
          sessionFilePath(task.cwd, nativeSessionId),
    ...
    const nativeQuery = yield* Effect.try({
      try: () =>
        query({
          ...
          options: {
            cwd: task.cwd,
            // This SDK process is headless and does not occupy the parent's
  ...
  restore: (task, meta) => {
    if (!meta.nativeSessionId) {
      return new SpawnError({
        message: "Claude recovery requires a native session id.",
      });
    }
    return makeClaudeSession(task, meta);
  },
};
