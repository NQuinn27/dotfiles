...
export function makeStubBackend(profile: StubProfile): SubagentBackend {
  return {
    ...
    restore: (task, meta) => makeStubSession(profile, task, { meta }),
  };
}
...
const makeStubSession = (
  profile: StubProfile,
  task: SpawnTask,
  options: { readonly meta?: SubagentMeta },
): Effect.Effect<SubagentSession, never, Scope.Scope> =>
  Effect.gen(function* () {
    const sessionId =
      options.meta?.nativeSessionId ??
    ...
    const state = {
      meta: {
        ...
        modelLabel:
          options.meta?.modelLabel ?? task.model ?? profile.defaultModelLabel,
        contextWindow:
          options.meta?.contextWindow ?? profile.contextWindow,
        sessionFilePath: options.meta?.sessionFilePath ?? sessionFile,
    ...
    // Announce metadata. The manager durably records the recovery handle
    // before explicitly submitting the initial or continuation prompt.
