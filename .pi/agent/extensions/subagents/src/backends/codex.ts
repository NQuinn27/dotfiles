  ...
  Effect.gen(function* () {
    if (!task.parent.projectTrusted) {
      return yield* new SpawnError({
        message:
          "Codex subagents require a Pi-trusted working directory because headless Codex runs without approval prompts.",
      });
    }
    ...
    const child = yield* Effect.try({
      try: () =>
        spawn(binary, ["app-server", "--stdio"], {
          cwd: task.cwd,
          // The app-server is headless and does not occupy the parent's
          // Herdr pane. Native hooks must not report against that pane.
          env: withoutHerdrEnvironment(),
    ...
    const state = {
      ...
      meta: {
        backend: "codex",
        modelLabel: recoveryMeta?.modelLabel ?? task.model,
        contextWindow: recoveryMeta?.contextWindow,
        nativeSessionId: recoveryMeta?.nativeSessionId,
        sessionFilePath: recoveryMeta?.sessionFilePath,
    ...
    const threadResult = yield* Effect.tryPromise({
      try: async () => {
        ...
        // already chose to launch an autonomous subagent, so preserve the
        // same autonomous policy when reopening a persisted thread.
        if (recoveryMeta?.nativeSessionId) {
          return request("thread/resume", {
            threadId: recoveryMeta.nativeSessionId,
            cwd: task.cwd,
    ...
    state.meta = {
      ...
      sessionFilePath:
        stringValue(thread?.path) ?? recoveryMeta?.sessionFilePath,
      nativeSessionId,
    };
  ...
  restore: (task, meta) => {
    if (!meta.nativeSessionId) {
      return new SpawnError({
        message: "Codex recovery requires a native thread id.",
      });
    }
    return makeCodexSession(task, meta);
  },
};
