...
import { withoutHerdrEnvironment } from "../herdr/environment.ts";
...
async function createChildResources(cwd: string, projectTrusted: boolean) {
  ...
  const loader = new DefaultResourceLoader({
    ...
    agentDir,
    settingsManager,
    // An in-process child shares the parent's process environment, including
    // Herdr caller ids. Do not teach a headless child to control that pane.
    skillsOverride: (base) => ({
      ...base,
      skills: base.skills.filter((skill) => skill.name !== "herdr"),
    }),
  });
  await loader.reload();
...
const makePiSession = (
  ...
  Effect.gen(function* () {
    ...
    // On recovery the persisted Pi session is authoritative for model and
    // thinking level. Passing current-parent defaults would silently change
    // the child after a parent model switch.
    const model = recoveryMeta
      ? undefined
      : yield* Effect.try({
          try: () =>
    ...
    const thinkingLevel = recoveryMeta
      ? undefined
    ...
    const session = yield* Effect.tryPromise({
      try: async () => {
        ...
        const sessionManager = recoveryMeta?.sessionFilePath
          ? SessionManager.open(recoveryMeta.sessionFilePath)
        ...
        const {
          ...
          spawnHook: (context) => ({
            ...context,
            env: withoutHerdrEnvironment(context.env),
          }),
        });
        const { session } = await createAgentSession({
          ...
          // Override the built-in Bash execution so subprocesses from this
          // in-process child cannot inherit the parent's Herdr pane authority.
          // Rendering is inherited from the built-in Bash tool.
          customTools: [childBashTool],
        });
...
export const piBackend: SubagentBackend = {
  ...
  spawn: makePiSession,
  restore: (task, meta) => {
    if (!meta.sessionFilePath) {
      return new SpawnError({
        message: "Pi recovery requires a child session file.",
      });
    }
    return makePiSession(task, meta);
  },
};
