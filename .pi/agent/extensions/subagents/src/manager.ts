...
export interface SubagentManagerShape {
  spawn(
    backend: BackendName,
    task: SpawnTask,
    onReady?: (snapshot: SubagentSnapshot) => void,
  ): Effect.Effect<
    SubagentSnapshot,
  ...
  /** Rehydrate durable snapshots without launching native processes. */
  restore(records: ReadonlyArray<RecoveredSubagent>): Effect.Effect<void>;
  /** Reopen a native conversation and explicitly start a continuation turn. */
  resume(
    id: string,
    task: SpawnTask,
    text: string,
    ...
    void,
    SpawnError | SendError | ConcurrencyLimitError | BackendUnavailableError
  >;
...
const makeManager = Effect.gen(function* () {
  ...
  const nextId = (origin: SubagentOrigin) => {
    const prefix = origin === "btw" ? "btw" : "sa";
    let id: string;
    do id = `${prefix}-${randomUUID().slice(0, 8)}`;
    ...
    Effect.gen(function* () {
      ...
      const doSpawn = Effect.gen(function* () {
        ...
        const session = yield* Effect.uninterruptible(
          Scope.provide(backend.spawn(task), scope),
        ...
        yield* attachPump(entry, session, scope);

        const snapshot = entry.snapshot as SubagentSnapshot;
        yield* Effect.try({
          try: () => onReady?.(snapshot),
          catch: (error) =>
            new SpawnError({
              message:
          ...
          Effect.andThen(
            session.send(task.prompt).pipe(
              Effect.mapError(
                (error) => new SpawnError({ message: error.message }),
              ),
            ),
          ),
          Effect.onError(() => {
      ...
      // Session allocation, durable-ready callback, pump attachment, and
      // initial submission form one ownership handoff. Defer interruption
      // until the manager either owns the scope or has closed it.
      return yield* Effect.uninterruptible(doSpawn).pipe(
        Effect.ensuring(
          Effect.sync(() => {
            reserved--;
            notify();
  ...
  const restore = (records: ReadonlyArray<RecoveredSubagent>) =>
    Effect.sync(() => {
      for (const record of records) {
        if (entries.has(record.snapshot.id)) continue;
        const snapshot: MutableSnapshot = {
          ...
          liveAssistant: record.snapshot.liveAssistant
            ? { ...record.snapshot.liveAssistant }
            : undefined,
          liveTools: [...record.snapshot.liveTools],
          queued: [...record.snapshot.queued],
        };
        entries.set(snapshot.id, {
          snapshot,
          liveToolMap: new Map(
            snapshot.liveTools.map((tool) => [tool.toolId, tool]),
          ),
        });
        notify(snapshot.id);
      }
  ...
  const resume = (id: string, task: SpawnTask, text: string) =>
    Effect.gen(function* () {
      ...
      if (entry.session) return yield* send(id, text);

      yield* Effect.suspend(
        (): Effect.Effect<void, ConcurrencyLimitError | SpawnError> => {
          if (disposed) {
            return new SpawnError({
              message: "Subagent manager is shutting down.",
            });
          ...
          if (runningCount() + reserved >= MAX_RUNNING) {
            return new ConcurrencyLimitError({
              message: `Max ${MAX_RUNNING} subagents can run concurrently; resuming "${id}" would exceed that.`,
            });
          }
          reserved++;
          return Effect.void;
      ...
      const work = Effect.gen(function* () {
        ...
        const session = yield* Effect.uninterruptible(
          Scope.provide(
            backend.restore(task, entry.snapshot.meta),
            scope,
          ),
        ).pipe(Effect.onError(() => Scope.close(scope, Exit.void)));
        yield* attachPump(entry, session, scope);
        ...
        yield* session.send(text).pipe(
          Effect.onError(() =>
            Effect.gen(function* () {
              entry.snapshot.status = previous.status;
              entry.snapshot.generation = previous.generation;
              entry.snapshot.settledAt = previous.settledAt;
              entry.snapshot.errorText = previous.errorText;
      ...
      // As with spawn, do not allow cancellation between native restoration,
      // scope ownership, and continuation submission.
      return yield* Effect.uninterruptible(work).pipe(
        Effect.ensuring(
          Effect.sync(() => {
            reserved--;
            notify(id);
  ...
  const send = (id: string, text: string) =>
    Effect.suspend((): Effect.Effect<void, SendError> => {
      ...
      if (!session) {
        return new SendError({
          message: `Subagent "${id}" was restored after a parent restart. Use subagent_resume before sending more work.`,
        });
      }
      ...
      if (entry.snapshot.status !== "running") {
        ...
        // Occupy the slot synchronously: the RunStarted event is folded by
        // the async pump, and concurrent restarts must not race past the cap.
        const previous = {
          status: entry.snapshot.status,
          generation: entry.snapshot.generation,
          settledAt: entry.snapshot.settledAt,
