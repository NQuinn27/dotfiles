...
export interface SubagentBackend {
  ...
  /**
   * Create a new idle native session. The manager durably records its handle
   * before calling `send()` with the initial prompt. Scoped: closing the scope
   * interrupts/kills the underlying session or process and ends `events`.
   */
  spawn(
    task: SpawnTask,
  ): Effect.Effect<SubagentSession, SpawnError, Scope.Scope>;
  /**
   * Reopen an existing native conversation without starting a new turn.
   * The caller must explicitly send a continuation after recovery.
   */
  restore(
    task: SpawnTask,
    meta: SubagentMeta,
