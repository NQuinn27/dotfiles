...
test(
  "Codex backend resumes its native thread in a new manager",
  { timeout: 120_000 },
  async (t) => {
    if (!(await codexAvailable())) {
      t.skip("codex executable is unavailable");
    ...
    try {
      ...
      const started = await runTool(
        ...
        manager.spawn(
          "codex",
          task("Remember the marker CEDAR-4826 and reply only: ready"),
        ),
      );
      await deadline(
    ...
    try {
      ...
      assert.match(manager.view.get(persisted.id)?.finalText ?? "", /CEDAR-4826/);
    } finally {
