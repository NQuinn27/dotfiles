...
10. **Persistence across reloads — implemented first recovery slice.** The extension
    now checkpoints per-parent run snapshots, native conversation handles, terminal
    results, and delivery receipts under the private Pi agent state directory. A
    resumed parent restores its runs; previously active runs become `recoverable` and
    require explicit `subagent_resume`, which opens the Pi session, Claude session, or
    Codex thread and starts a new continuation turn. Graceful shutdown still stops live
    backend processes, and a hard crash does not preserve an in-flight model/tool
    operation. True live reattachment remains future detached-worker work.
