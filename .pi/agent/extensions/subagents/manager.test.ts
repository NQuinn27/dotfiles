...
test("spawn records its native handle before submitting the initial prompt", async () => {
  await withManager(async (manager, runtime) => {
    let ready = false;
    const snap = await runTool(
      runtime,
      manager.spawn("claude", task("Start only after the ledger callback"), (snapshot) => {
        ready = true;
        assert.ok(snapshot.meta.nativeSessionId);
        assert.equal(
          snapshot.transcript.some((item) => item.kind === "user"),
          false,
        );
      }),
    );
...
test("a failed durable-ready callback prevents the initial run", async () => {
  await withManager(async (manager, runtime) => {
    await assert.rejects(
      runTool(
        runtime,
        manager.spawn("claude", task("must not run"), () => {
...
test("restored snapshots require explicit native resume", async () => {
  let persisted: NonNullable<ReturnType<SubagentManagerShape["view"]["get"]>>;
  const firstRuntime = createTestRuntime();
  try {
    const firstManager = await firstRuntime.runPromise(SubagentManager);
    const started = await runTool(
      firstRuntime,
      firstManager.spawn("claude", task("Original turn")),
    );
    await runTool(firstRuntime, firstManager.waitFor([started.id]));
    const settled = firstManager.view.get(started.id);
    assert.ok(settled);
    persisted = {
      ...settled,
      status: "recoverable",
  ...
  try {
    ...
    await runTool(
      secondRuntime,
      manager.restore([{ snapshot: persisted, task: task("Original turn") }]),
    );
    assert.equal(manager.view.get(persisted.id)?.status, "recoverable");
    await assert.rejects(
      runTool(secondRuntime, manager.send(persisted.id, "unsafe implicit send")),
      /subagent_resume/,
    );
    ...
    await runTool(secondRuntime, manager.waitFor([persisted.id]));
    const resumed = manager.view.get(persisted.id);
    assert.equal(resumed?.status, "done");
    assert.match(resumed?.finalText ?? "", /Continue after recovery/);
  } finally {
